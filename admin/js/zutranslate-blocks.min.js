var zutranslate_blocks = (function (exports) {
  ...
  var _select$3 = select$4('core/block-editor'),
      getBlockOrder = _select$3.getBlockOrder; // recursively collect all IDs of 'editable blocks' on the page.
  ...
  var _lodash$9 = lodash,
      keys$3 = _lodash$9.keys,
      isEqual = _lodash$9.isEqual,
      get$2 = _lodash$9.get,
      omit = _lodash$9.omit,
      without = _lodash$9.without;
  var registerStore = wp.data.registerStore; // Internal dependencies
  var editorLang = getExternalData('lang', 'en');
  var enableDebug$6 = getExternalData('debug.raw_store', false);
  var debug$6 = getDebug(enableDebug$6); // Create and register Zu Translate store -------------------------------------]
  ...
  function storeReducer() {
    ...
    var type = action.type,
        key = action.key,
        value = action.value;
    var prevValue = get$2(state, key, '');
    var lang = get$2(state, 'lang', ''); // use an interim value to avoid unnecessary rendering
  ...
  var store = registerStore(ZUTRANSLATE_STORE, {
    ...
    selectors: {
      getRaw: function getRaw(state, key) {
        return get$2(state, key);
      },
      getLang: function getLang(state) {
        return get$2(state, 'lang');
      },
      getHooks: function getHooks(state) {
        return get$2(state, 'hooks');
      },
      getWatched: function getWatched(state) {
        return get$2(state, 'watched');
      }
    },
    controls: {}
  });
  var subscribe$2 = store.subscribe;

  // WordPress dependencies
  var _lodash$8 = lodash,
      keys$2 = _lodash$8.keys,
      forEach$4 = _lodash$8.forEach,
      includes$2 = _lodash$8.includes;
  var _wp$data$3 = wp.data,
      select$3 = _wp$data$3.select,
      dispatch$2 = _wp$data$3.dispatch; // Internal dependencies
  var enableDebug$5 = getExternalData('debug.sync_blocks', false);
  var activateSync$1 = getExternalData('sync', false);
  var cleanUnsaved = getExternalData('unsaved', false);
  var debug$5 = getDebug(enableDebug$5);
  ...
  function getWatched() {
    return select$3(ZUTRANSLATE_STORE).getWatched();
  }
  ...
  function addWatched(id) {
    ...
    debug$5.infoWithId(id, "-+{Component Watched}".concat(isOriginator ? ' [originator]' : ''));
    addWatched(id);
  }
  ...
  function removeWatched(id) {
    ...
    debug$5.infoWithId(id, "-".concat(count < 0 ? '!' : '*', "{Component will be unWatched}, remained in the list [").concat(count, "]"));
    removeWatched(id);
  }

  function getHooks() {
    return select$3(ZUTRANSLATE_STORE).getHooks();
  }
  ...
  function syncBlocks() {
    ...
    if (activateSync$1) {
      var hooks = getHooks();
      forEach$4(hooks, function (hook, id) {
        if (id !== clientId) {
  ...
  function syncCompleted() {
    ...
    if (includes$2(watched, id)) removeWatched(id);else refreshStore();
  }
  ...
  function debugSync(when, isSyncEnabled, mode) {
    var isPostDirty = entityState.isPostDirty,
        ...
        shouldResetEdits = entityState.shouldResetEdits;
    var published = isPostPublished ? 'published' : 'not published';
    var isBefore = when === 'before';
    var status = isPostDirty ? 'dirty' : 'clean';
    var syncMode = mode ? ', without originator' : '';
    var option = (isSyncEnabled ? 'enabled' : mode ? 'single mode' : 'disabled') + syncMode;
    var action = "".concat(isBefore ? '?' : '#', "{").concat(isBefore ? 'initiated' : 'completed', "}");
    var resetDisabled = "clean \"unsaved\" is ".concat(isPostPublished ? 'disabled' : 'not possible');
    var resetNote = shouldResetEdits && isTracking ? 'enabled' : isTracking ? 'disabled' : 'not tracked';
    var reset = cleanUnsaved && isPostPublished ? "reset is {".concat(resetNote, "}") : resetDisabled;
    var after = isBefore ? ", \"Watched\" count [".concat(keys$2(getWatched()).length, "]") : isPostDirty ? ", ".concat(reset) : '';
    var info = "-".concat(action, " Sync Blocks [sync ").concat(option, "] - Post [").concat(published, "] and is {").concat(status, "}").concat(after);
    debug$5.info(info);
  }

  // WordPress dependencies
  var _lodash$7 = lodash,
      keys$1 = _lodash$7.keys,
      forEach$3 = _lodash$7.forEach,
  ...
  var _wp$data$2 = wp.data,
      ...
      dispatch$1 = _wp$data$2.dispatch;
  var _wp = wp,
      apiFetch = _wp.apiFetch; // Internal dependencies
  var supportSession$1 = getExternalData('session', false);
  var enableDebug$4 = getExternalData('debug.sync_blocks', false);
  var debug$4 = getDebug(enableDebug$4); // Custom hooks & helpers for 'store' -----------------------------------------]

  function getLang() {
    return select$2(ZUTRANSLATE_STORE).getLang();
  }
  function getRaw(key) {
    return select$2(ZUTRANSLATE_STORE).getRaw(key);
  }
  ...
  function addHook(id, hook) {
    var hooks = select$2(ZUTRANSLATE_STORE).getHooks();
  ...
  function useOnLangChange(clientId, callback, prevLang) {
    ...
    useEffect$2(function () {
      ...
      debug$4.infoWithId(clientId, "-#on lang change : {will ".concat(enable ? 'switch' : 'skip', "}"), {
        prev: prev,
  ...
  var _select$2 = select$2('core/editor'),
      isSavingPost$1 = _select$2.isSavingPost;

  apiFetch.use(function (options, next) {
    if (isSavingPost$1() && includes$1(['PUT', 'POST'], options.method)) {
      ...
      if (some(keys$1(data), function (val) {
        return includes$1(supportedKeys, val);
      })) {
        forEach$3(supportedKeys, function (attr) {
          if (has$3(data, attr)) {
            var rawValue = getRaw(attr);
            set$1(newOptions, ['data', attr], rawValue);
          }
        });
      }
  ...
  var _lodash$6 = lodash,
      isEmpty = _lodash$6.isEmpty,
      keys = _lodash$6.keys,
      pick = _lodash$6.pick,
      get$1 = _lodash$6.get,
      set = _lodash$6.set,
      forEach$2 = _lodash$6.forEach,
      every = _lodash$6.every,
      has$2 = _lodash$6.has,
      reduce = _lodash$6.reduce;
  var _wp$data$1 = wp.data,
      subscribe$1 = _wp$data$1.subscribe,
      select$1 = _wp$data$1.select,
      dispatch = _wp$data$1.dispatch; // Internal dependencies
  ...
  var enableDebug$3 = getExternalData('debug.edited_entity', false);
  var debug$3 = getDebug(enableDebug$3); // local storage for current attribute values

  var editedAttributes = null; // Attributes of the Entity ---------------------------------------------------]
  ...
  function initEditedAttribute(attr, listener) {
    ...
    setEditedAttributes(_defineProperty({}, attr, get$1(atts, attr)));
    ...
    if (editedAttributes.isReady) debug$3.info('-+{ready} Edited Attributes', editedAttributes);
    return get$1(atts, rawAttr);
  }
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      debug$3.info('-+{updated} Entity Attributes', edits);
      var postType = getCurrentPostType();
      var postId = getCurrentPostId$1(); // definitely need to be called before 'editEntityRecord'
  ...
  function setEditedAttributes(edits) {
    ...
    forEach$2(edits, function (value, attr) {
  ...
  function updateEditedAttributes(edits) {
    forEach$2(edits, function (value, attr) {
      ...
      listener(attr, value);
      debug$3.info("-*updated RAW for is ".concat(attr, " - [").concat(value, "]"));
    });
  }
  ...
  function diffEdits(edits) {
    ...
    var entityAtts = keys(edits).length !== keys(prevEdits).length ? getEntityAttributes() : {};
    ...
    var hasChanges = keys(changes).length > 0;
  ...
  subscribe$2(function () {
    ...
    if (entityState.isTracking && changesAreCommitted()) {
      ...
      debug$3.info('-#RAW store tracking is {completed}', entityState);
      resetEdits();
    }
  });
  ...
  function testEdits() {
    if (hasEditedAttributes()) {
      var edits = pick(getNonTransientEdits(), supportedKeys);
      var changes = diffEdits(edits); // debug.data({ changes })

      return changes ? [true, changes] : [false]; // 	const editKeys = keys(edits);
  ...
  function resetEdits() {
    ...
    debug$3.info('-?{resetEdits}', keys(nonTransientEdits));
  ...
  function emulateSavingPost(postEdits) {
    ...
    debug$3.info("-*Emulate {Saving Post} [".concat(postId, "]"), nonTransientEdits);
  ...
  subscribe$1(function () {
    if (isSavingPost()) {
      postSaved = false;
      debug$3.info('-?{Saving Post...}');
    } else {
      if (!postSaved) {
        debug$3.info('-*{Post Saved}');
        postSaved = true;
      }
    }
  });
  ...
  function debugPostStatus() {
    if (enableDebug$3) {
      ...
      var published = isPostPublished ? 'published' : 'not published';
      var status = isPostDirty ? 'dirty' : 'clean';
      var args = ["-".concat(isPostDirty ? '!' : '*', "Post [").concat(published, "] and editing state is {").concat(status, "}")];
      !shouldResetEdits && args.push(keys(getNonTransientEdits()));
      debug$3.info.apply(debug$3, args);
    }
  }
  ...
  var _lodash$5 = lodash,
      ...
      isString = _lodash$5.isString; // Internal dependencies
  var enableDebug$2 = getExternalData('debug.raw_helpers', false);
  var debug$2 = getDebug(enableDebug$2);
  var rawElements = {
  ...
  function replaceRawElements() {
    forEach$1(rawElements, function (selectors, element) {
  ...
  function attachInsertedHooks(element, root, selector) {
    whenNodeInserted(root, selector, function (node) {
      ...
      debug$2.info("-?Node Inserted for \"{".concat(element, "}\" - RAW {").concat(replaced ? 'replaced' : 'not found', "}"));
    });
  }

  // WordPress dependencies
  var _lodash$4 = lodash,
      forEach = _lodash$4.forEach,
      castArray = _lodash$4.castArray; // , noop, includes
  ...
  function setRawAttributes() {
    ...
    forEach(supportedKeys, function (attr) {
      ...
      if (value === undefined) {
        ...
        debug$1.info("-?{init} RAW for [".concat(attr, "]"), value);
    ...
    if (initAttributes) {
      ...
      if (sessionLang && rootLangSetter) {
        debug$1.info("-?{lang check} session/editor [".concat(sessionLang, "/").concat(editorLang, "]"));
    ...
    forEach(attributes, function (value, attr) {
      ...
      if (rawValue !== undefined) {
        ...
        if (value !== shouldBeValue) {
          ...
          debug$1.info("-#{switch} RAW [".concat(attr, "] for lang {").concat(editorLang, "}"), shouldBeValue);
        }
      }
    });
  ...
  var _lodash$3 = lodash,
      includes = _lodash$3.includes; // const { __ } = wp.i18n;
  ...
  var BlockEditLang = function BlockEditLang(props) {
    ...
    if (rawRef.current === null) {
      ...
      debug.infoWithId(clientId, "-#Initiated with language {".concat(qtxLang, "}"));
    ...
    var replaceContent = useCallback(function (lang) {
      ...
      if (lang !== prevLang) {
        var atts = switchContent(raw, lang, translatedAtts); // rawRef.current.lang = lang;

        debug.infoWithId(id, "-#{switching} RAW [".concat(translatedAtts, "] for lang {").concat(lang, "}"), atts);
        setAttributes(_objectSpread2({
          qtxLang: lang
        }, atts));
      } else {
        debug.infoWithId(id, "--{skip switching} RAW [".concat(translatedAtts, "]"), {
          lang: lang,
          prevLang: prevLang,
    ...
    var onChangeLang = useCallback(function (lang) {
      ...
      debug.infoWithId(id, "-Language switched [".concat(qtxLang, " -> ").concat(lang, "]"));
    }, [forceUpdate, replaceContent, qtxLang]); // synchronize, create RAW if does not exist and maybe fix it - on mounting only
    ...
    useEffect(function () {
      ...
      if (!hasRaw(rawRef)) {
        ...
        rawRef.current.raw = raw; // rawRef.current.lang = editorLang;
    ...
    useEffect(function () {
      if (hasRaw(rawRef)) {
        var raw = rawRef.current.raw; // here we cannot use lang from 'rawRef' because 'translatedValues' are created based on 'qtxLang'
        ...
        if (updatedRaw !== raw) {
          ...
          debug.infoWithId(rawRef.current.id, '-#Raw {updated}', {
    ...
    }, [].concat(_toConsumableArray(translatedValues), [qtxLang, setAttributes])); // const controlLang = qtxLang;
  ...
  var _lodash$2 = lodash,
      ...
      assign = _lodash$2.assign,
      has = _lodash$2.has; // const { __ } = wp.i18n;
  ...
  function resetPostEditorAutosave() {
    ...
    if (!get(wp, ['zukit', resetKey])) {
      ...
      var resetAutosave = function resetAutosave() {
        if (hasSessionStorageSupport()) {
          ...
          window.console.warn("ZUKIT: The backup of the post [".concat(postId, "] in your browser was removed"));
        }
      };
    }
  } // Function which returns true if the current environment supports browser
