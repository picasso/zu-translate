var zutranslate_blocks = (function (exports) {
  ...
  function removeWatched(id) {
    ...
    if (!includes$2(watched, id)) debug$4.infoWithId(id, '-!{Repeated ID removal!}');
  ...
  function syncBlocks() {
    ...
    if (activateSync$1) {
      var hooks = getHooks();
      forEach$2(hooks, function (hook, id) {
        if (id !== clientId) {
          // always add watched ID before calling the hook, because inside the hook may be logic
          // to remove this ID from the 'watched' list
          // for example, in the language switch logic for 'non-block' attributes
  ...
  var debug$3 = getDebug(enableDebug$3); // Attributes of the Entity ---------------------------------------------------]
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      debug$3.info('-+{updated} Entity Attributes', edits); // collectEdits(edits);
  ...
  function setRawAttributes() {
    ...
    var langSetter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    ...
    forEach(supportedAtts, function (selector, attr) {
      if (addListeners) {
        ...
        if (value === undefined) {
          ...
          setRaw(attr, value);
          addInputListener(selector, getListener(attr)); // 'title' does not require 'inserted hook' as it is not removed from the page
  ...
  var LangControlSetting = function LangControlSetting(_ref) {
    ...
    useEffect$1(function () {
      ...
      registerRootUpdater(rootClientId); // set the initial RAW attributes on mounting the component and add listeners
