var zutranslate_blocks = (function (exports) {
  ...
  var debug$4 = getDebug(enableDebug$4); // Create and register Zu Translate store -------------------------------------]
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      debug$3.info('-Update {Entity Attributes}', edits); // collectEdits(edits);

      var postType = getCurrentPostType();
      var postId = getCurrentPostId();
      editEntityRecord('postType', postType, postId, edits);
      debug$3.infoWithId(rootClientId, '-*{Component unWatched}');
  ...
  subscribe(function () {
    if (isEditedPostDirty()) {
      if (!isPostDirty) {
        isPostDirty = true;
        debugPostStatus();
        if (!shouldResetEdits) debug$3.data({
  ...
  subscribe$1(function () {
    if (isTracking && completedTracking()) {
      isTracking = false;
      debug$3.info('-?RAW store tracking is {completed}', {
  ...
  function afterLanguageSwitch(lang) {
  ...
  function emulateSavingPost(postEdits) {
    ...
    debug$3.info('-*Emulate {Saving Post}', postId, nonTransientEdits);
    receiveEntityRecords('postType', postType, updatedRecord, undefined, true, edits);
  }
  function storeTest() {
    debug$3.info('-?storeTest - Activated manually');
    emulateSavingPost();
  ...
  function debugPostStatus(message, more) {
    if (enableDebug$3) {
      ...
      if (more) debug$3.info(info, cloneDeep(more));else debug$3.info(info);
    }
  }
  ...
  function debugLanguageSwitch(when, lang) {
    if (enableDebug$3) {
      var status = isPostDirty ? 'dirty' : 'clean';
      var info = "-".concat(when === 'before' ? '?' : '+', "{").concat(when, "} Language Switch [").concat(lang, "] - Post is {").concat(status, "}"); // if(more) debug.info(info, cloneDeep(more));
      // else 
  ...
  var debug$2 = getDebug(enableDebug$2); // Custom hooks & helpers for 'store' -----------------------------------------]
  ...
  function addWatched(id) {
    ...
    addWatched(id);
    debug$2.infoWithId(id, "-!{Component Watched}".concat(isOriginator ? '- [originator]' : ''));
  }
  function removeWatched(id) {
    ...
    removeWatched(id);
    debug$2.infoWithId(id, '-*{Component unWatched}');
  ...
  function syncBlocks(clientId) {
    ...
    if (activateSync$1) {
      var hooks = getHooks();
      debug$2.infoWithId(clientId, '-Sync initiated', {
