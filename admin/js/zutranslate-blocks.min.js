var zutranslate_blocks = (function (exports) {
  ...
  function debugSync(when, isSyncEnabled, mode) {
    ...
    var resetNote = sprintf$1('reset is {%s}', isTracking ? shouldResetEdits ? 'enabled' : 'disabled' : 'not tracked');
    var disableNote = sprintf$1('clean "unsaved" is {%s}', isPostPublished ? 'disabled' : 'not possible');
    var info = sprintf$1('-%1$s{%2$s} Sync Blocks [sync %3$s%4$s] - Post [%5$s] and is {%6$s}%7$s%8$s', isBefore ? '?' : '#', // 1
    ...
    isBefore ? ", \"Watched\" count [".concat(keys$2(getWatched()).length, "]") : '', // 7
  ...
  var enableDebug$4 = getExternalData('debug.raw_helpers', false);
  ...
  function debugPostStatus() {
    if (enableDebug$3) {
      var isPostPublished = entityState.isPostPublished,
          ...
          shouldResetEdits = entityState.shouldResetEdits;
      var args = [sprintf('-%sPost [%s] and editing state is {%s}', isPostDirty ? '!' : '*', isPostPublished ? 'published' : 'not published', isPostDirty ? 'dirty' : 'clean')];
  ...
  var _lodash$4 = lodash,
      ...
      castArray = _lodash$4.castArray; // Internal dependencies
