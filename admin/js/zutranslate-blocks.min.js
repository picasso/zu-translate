var zutranslate_blocks = (function (exports) {
  ...
  function debugSync(when, isEnabled) {
    ...
    var action = "".concat(when === 'before' ? '?' : '#', "{").concat(when === 'before' ? 'initiated' : 'completed', "}");
    var hooksCount = "Hooks count [".concat(keys$1(getHooks()).length, "]");
    var info = "-".concat(action, " Sync Blocks [").concat(option, "] - Post is {").concat(status, "}, ").concat(hooksCount);
    debug$3.info(info);
  }
  ...
  var _lodash$4 = lodash,
      ...
      keys = _lodash$4.keys,
      pick = _lodash$4.pick,
      cloneDeep = _lodash$4.cloneDeep; // isArray, includes, pull, some,
  ...
  var debug$2 = getDebug(enableDebug$2); // Attributes of the object Entity --------------------------------------------]
  ...
  subscribe(function () {
    if (isEditedPostDirty()) {
      if (!entityState.isPostDirty) {
        ...
        debugPostStatus(!entityState.shouldResetEdits ? {
  ...
  function debugPostStatus(message, more) {
    if (enableDebug$2) {
      ...
      var info = "-".concat(entityState.isPostDirty ? '!' : '*').concat(desc, "Post is {").concat(status, "}");
