var zutranslate_blocks = (function (exports) {
  ...
  function removeWatched(id) {
    ...
    if (!includes$1(watched, id)) debug$3.infoWithId(id, '-!{Repeated ID removal!}');
  ...
  var debug$2 = getDebug(enableDebug$2); // local storage for current attribute values

  var entityAttributes = null; // Attributes of the Entity ---------------------------------------------------]
  ...
  function initEditedAttribute(attr, listener) {
    ...
    entityAttributes.isReady = every(supportedKeys, function (attr) {
      return has$2(entityAttributes, attr);
    });
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      debug$2.info('-+{updated} Entity Attributes', edits);
      var postType = getCurrentPostType();
  ...
  function getEditedChanges() {
    var edits = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    var updated = null;

    if (entityAttributes) {
      if (edits) {
        updated = reduce(edits, function (acc, value, attr) {
          if (value !== get(entityAttributes, [attr, 'value'])) acc[attr] = value;
          return acc;
        }, {});
      } else {
        var atts = getEntityAttributes();
        updated = reduce(atts, function (acc, value, attr) {
          if (value !== get(entityAttributes, [attr, 'value'])) acc[attr] = value;
          return acc;
        }, {});
      }
  ...
  function setEditedAttributes(edits) {
    forEach$2(edits, function (value, attr) {
      return set(entityAttributes, [attr, 'value'], value);
    });
  }

  function updateEditedAttributes(edits) {
    forEach$2(edits, function (value, attr) {
      var listener = entityAttributes[attr].listener;
  ...
  function testEdits() {
    if (hasEditedAttributes()) {
      ...
      var editKeys = keys(edits);
      var hasKeys = editKeys.length > 0;

      if (hasKeys || !hasKeys && entityState.wasDirty) {
        // if(entityState.isClear) debug.info('!extra check!');
        var updated = getEditedChanges(hasKeys ? edits : null); // if(!hasKeys && entityState.wasDirty) entityState.isClear = false;
  ...
  function attachInsertedHooks(element, root, selector) {
    whenNodeInserted(root, selector, function (node) {
      ...
      debug$1.info("-?Node Inserted for \"{".concat(element, "}\" - RAW {").concat(replaced ? 'replaced' : 'not found', "}"));
    });
  }
  ...
  var sessionLang = supportSession ? getSessionLang() : null; // const enableDebug = getExternalData('debug.raw_helpers', false);
