var zutranslate_blocks = (function (exports) {
  ...
  // WordPress dependencies
  var _$1 = lodash;
  var select$3 = wp.data.select; // Zukit dependencies
  ...
  var _select$2 = select$3('core/block-editor'),
  ...
  var _lodash$6 = lodash,
      ...
      isEqual = _lodash$6.isEqual,
      get = _lodash$6.get,
      omit = _lodash$6.omit,
      without = _lodash$6.without;
  ...
  var enableDebug$4 = getExternalData('debug.raw_store', false);
  var debug$3 = getDebug(enableDebug$4); // Create and register Zu Translate store -------------------------------------]

  var supportedAtts = {
    title: '.editor-post-title__input',
    excerpt: '.editor-post-excerpt__textarea .components-textarea-control__input'
  };
  var supportedKeys = keys$2(supportedAtts);
  ...
  var _lodash$5 = lodash,
      isEmpty = _lodash$5.isEmpty,
      isArray = _lodash$5.isArray,
      keys$1 = _lodash$5.keys,
      pick = _lodash$5.pick,
      includes$3 = _lodash$5.includes,
      pull = _lodash$5.pull,
      some$1 = _lodash$5.some,
      cloneDeep = _lodash$5.cloneDeep; // const { useEffect  } = wp.element; // useCallback, useRef
  ...
  var _select$1 = select$2('core'),
      getEntityRecordNonTransientEdits = _select$1.getEntityRecordNonTransientEdits,
      getEditedEntityRecord = _select$1.getEditedEntityRecord;

  var _dispatch = dispatch$1('core'),
      editEntityRecord = _dispatch.editEntityRecord,
      receiveEntityRecords = _dispatch.receiveEntityRecords;

  var _select2 = select$2('core/editor'),
  ...
  var _select3 = select$2('core/editor'),
      isSavingPost$1 = _select3.isSavingPost;

  var enableDebug$3 = getExternalData('debug.edited_entity', false);
  var debug$2 = getDebug(enableDebug$3); // hasChangedContent,
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      debug$2.info('-Update {Entity Attributes}', edits);
      collectEdits(edits);
      var postType = getCurrentPostType();
      var postId = getCurrentPostId();
  ...
  function collectEdits(edits) {
    var canCollect = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    var editKeys = isArray(edits) ? edits : keys$1(edits);
    if (!isArray(shouldResetEdits) && canCollect) shouldResetEdits = [];

    if (isArray(shouldResetEdits)) {
      var _shouldResetEdits;

      if (includes$3(shouldResetEdits, 'atts')) pull(shouldResetEdits, 'atts');
      if (includes$3(edits, 'atts') && some$1(keepAttributes, function (key) {
        return includes$3(supportedKeys, key);
      })) pull(edits, 'atts');

      (_shouldResetEdits = shouldResetEdits).push.apply(_shouldResetEdits, _toConsumableArray(editKeys).concat(_toConsumableArray(keepAttributes)));
  ...
  var isPostDirty = false;
  subscribe(function () {
    if (isEditedPostDirty()) {
      if (!isPostDirty) {
        isPostDirty = true;
        debugPostStatus();
        if (!shouldResetEdits) debug$2.data({
          nonTransientEdits: getNonTransientEdits()
        });
      }
    } else {
      if (isPostDirty) {
  ...
  subscribe$1(function () {
    if (isTracking && completedTracking()) {
      isTracking = false;
      debug$2.info('-*RAW store tracking is {completed}', {
        shouldResetEdits: shouldResetEdits
      });
      resetEdits();
    }
  });
  var shouldResetEdits = false;
  function beforeLanguageSwitch() {
    debugLanguageSwitch('before');
    if (isPostDirty) return;
    shouldResetEdits = true;
  }
  function afterLanguageSwitch(clientId, activateSync) {
  ...
  function completedTracking() {
    var watched = select$2(ZUTRANSLATE_STORE).getWatched();
    return watched.length === 0;
  }

  function resetEdits() {
    var nonTransientEdits = getNonTransientEdits();
    debugPostStatus('{resetEdits}', keys$1(nonTransientEdits)); // { shouldResetEdits, nonTransientEdits }
  ...
  subscribe(function () {
    ...
    if (isArray(shouldResetEdits) && isEmpty(shouldResetEdits) && !isPostDirty) {
      ...
      shouldResetEdits = false; // isWaitingForClean = false;
    }
  });
  function emulateSavingPost(postEdits) {
    ...
    var nonTransientEdits = postEdits !== null && postEdits !== void 0 ? postEdits : getNonTransientEdits(postType, postId); // yield external_wp_data_["controls"].select('core', 'getEntityRecordNonTransientEdits', kind, name, recordId);

    var edits = _objectSpread2({
      id: postId
    }, nonTransientEdits);

    debug$2.info('-*Emulate {Saving Post}', postId, nonTransientEdits, updatedRecord); // debug.data({ edits, record: pick(updatedRecord, keys(nonTransientEdits)) });
  ...
  function debugPostStatus(message, more) {
    if (enableDebug$3) {
      ...
      var desc = message ? "".concat(message, " - ") : '';
      var info = "-".concat(isPostDirty ? '!' : '*').concat(desc, "Post is {").concat(status, "}");
      if (more) debug$2.info(info, cloneDeep(more));else debug$2.info(info);
    }
  }
  ...
  function debugLanguageSwitch(when, more) {
    if (enableDebug$3) {
      var status = isPostDirty ? 'dirty' : 'clean';
      var info = "-".concat(when === 'before' ? '?' : '+', "{").concat(when, " Language Switch} - Post is {").concat(status, "}");
      if (more) debug$2.info(info, cloneDeep(more));else debug$2.info(info);
    }
  ...
  var _wp$data = wp.data,
      ...
      dispatch = _wp$data.dispatch; // subscribe,

  var _wp = wp,
      apiFetch = _wp.apiFetch; // Internal dependencies
  var enableDebug$2 = getExternalData('debug.sync_blocks', false);
  var activateSync$1 = getExternalData('sync', false);
  var debug$1 = getDebug(enableDebug$2); // Custom hooks & helpers for 'store' -----------------------------------------]
  ...
  function getHooks() {
    return select$1(ZUTRANSLATE_STORE).getHooks();
  }
  ...
  function addHook(id, hook) {
    var _dispatch3 = dispatch(ZUTRANSLATE_STORE),
        setHook = _dispatch3.setHook;

    setHook(id, hook);
  }
  function addWatched(id) {
    var _dispatch4 = dispatch(ZUTRANSLATE_STORE),
        addWatched = _dispatch4.addWatched;

    addWatched(id);
  }
  function removeWatched(id) {
    var _dispatch5 = dispatch(ZUTRANSLATE_STORE),
        removeWatched = _dispatch5.removeWatched;

    removeWatched(id);
  } // custom hook which get dispatch method for 'lang' change

  function changeLang(value) {
    var _dispatch6 = dispatch(ZUTRANSLATE_STORE),
        setLang = _dispatch6.setLang;
  ...
  function useOnLangChange(clientId, callback) {
    ...
    useEffect$2(function () {
      if (prev !== undefined && prev !== editorLang) {
        ...
        debug$1.infoWithId(clientId, '-*{Component unWatched}');
      }
  ...
  function useLangHook(clientId, updater) {
    useEffect$2(function () {
      var _dispatch7 = dispatch(ZUTRANSLATE_STORE),
          setHook = _dispatch7.setHook,
          removeHook = _dispatch7.removeHook;
  ...
  function syncBlocks(clientId) {
    addWatched(clientId);
    debug$1.infoWithId(clientId, '-!{Component Watched} - master');

    if (activateSync$1) {
      var hooks = getHooks();
      debug$1.infoWithId(clientId, '-Sync initiated', {
      ...
      forEach$1(hooks, function (hook, id) {
        if (id !== clientId) {
          hook();
          addWatched(id);
          debug$1.infoWithId(id, '-!calling hook + {Component Watched}');
        }
      });
    }

    afterLanguageSwitch(clientId, activateSync$1);
  } // Hook on the post saving ----------------------------------------------------]

  var _select = select$1('core/editor'),
      isSavingPost = _select.isSavingPost;
  apiFetch.use(function (options, next) {
    if (isSavingPost() && includes$2(['PUT', 'POST'], options.method)) {
      {
  ...
  function addObserver(observer, fullSelector, inserted) {
    ...
    if (has$2(storage, selector)) removeObserver(selector, inserted);
  ...
  var debug = getDebug(enableDebug$1); // helpers for RAW attributes -------------------------------------------------]
    ...
    forEach(supportedAtts, function (selector, attr) {
      if (addListeners) {
        ...
        if (value === undefined) {
          ...
          debug.info("set for {".concat(attr, "} is ignored, current value = ").concat(value));
        }
      } else {
  ...
  function switchRawAttributes(lang) {
    ...
    onlyAtts = onlyAtts === null ? null : castArray(onlyAtts);
    var atts = getEntityAttributes(onlyAtts);
    var edits = {};
    forEach(atts, function (value, attr) {
      ...
      if (rawValue !== undefined) {
        ...
        if (value !== shouldBeValue) edits[attr] = shouldBeValue; // changeInputValue(selector, shouldBeValue, true);
      } // }
  ...
  function registerRootUpdater() {
    var rootClientId = 'rawRoot';
    var hooks = getHooks();

    if (!has$1(hooks, rootClientId)) {
  ...
  var sidebarRoot = '.edit-post-sidebar > .components-panel';

  function attachInsertedHooks(attr, selector) {
    whenNodeInserted(sidebarRoot, selector, function () {
      debug.info("-Node Inserted for {\"".concat(attr, "\"}")); // synchronize switching for newly created element

      switchRawAttributes(null, attr); // add the listener again (maybe the previous one was removed with the element or maybe not)
  ...
  var BlockEditLang = function BlockEditLang(props) {
    ...
    var onChangeLang = useCallback(function (lang) {
      ...
      if (enableDebug) Zubug.info("{".concat(id, "} Language switched to [").concat(prevLang, " -> ").concat(lang, "]"));
    ...
    useLangHook(clientId, forceUpdate); // synchronize, create RAW if does not exist and maybe fix it - on mounting only
    ...
    useEffect(function () {
      if (hasRaw(rawRef)) {
        var _rawRef$current3 = rawRef.current,
            raw = _rawRef$current3.raw,
            lang = _rawRef$current3.lang;
  ...
  var withRawEditControl = createHigherOrderComponent(function (BlockEdit) {
    return function (props) {
      ...
      if (enableDebug) {
        if (!isEditableBlock) Zubug.info("Block [".concat(name, "] with id {").concat(clientId, "} was skipped"), {
          editableBlocks: editorIds
        });else Zubug.info("Block [".concat(name, "] isEditable and {").concat(isSupported(name) ? 'isSupported' : 'is NOT Supported', "}"));
      }
