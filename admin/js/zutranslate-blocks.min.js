var zutranslate_blocks = (function (exports) {
  ...
  // WordPress dependencies
  var _$1 = lodash;
  var select$2 = wp.data.select; // Zukit dependencies
  ...
  var _select$1 = select$2('core/block-editor'),
      getBlockOrder = _select$1.getBlockOrder; // recursively collect all IDs of 'editable blocks' on the page.
  ...
  var enableDebug$3 = getExternalData('debug.raw_store', false); // Create and register Zu Translate store -------------------------------------]

  var supportedAtts = {
    title: '.editor-post-title__input',
    excerpt: '.editor-post-excerpt__textarea .components-textarea-control__input'
  };
  var supportedKeys = keys$1(supportedAtts);
  ...
  function storeReducer() {
    ...
    if (enableDebug$3) Zubug.data({
      ...
      key: key,
      value: value,
      isEqual: isEqual$1(state, interim)
    });
    return isEqual$1(state, interim) ? state : interim;
  }
  ...
  apiFetch.use(function (options, next) {
    ...
    if (isSavingPost() && includes$2(['PUT', 'POST'], options.method)) {
      if (skipSavingPost) {
        ...
        var data = getCurrentPost();
        var edited = getEditedPost(); // const result = next(options);

        Zubug.data({
          data: data,
          edited: edited,
          isEqual: isEqual(edited, data)
        });
        return Promise.resolve( // we don't have headers because we "emulate" this request
        // new window.Response(
        ...
        var newOptions = _objectSpread2(_objectSpread2({}, options), {}, {
          data: _objectSpread2(_objectSpread2({}, _data), {}, {
            editor_lang: getLang()
          })
        });

        if (some(keys(_data), function (val) {
  ...
  var _select2 = select$1('core/editor'),
      ...
      getPostEdits = _select2.getPostEdits,
      getCurrentPostType = _select2.getCurrentPostType,
      getCurrentPostId = _select2.getCurrentPostId; // hasChangedContent,
  ...
  var _select3 = select$1('core'),
      getEntityRecordEdits = _select3.getEntityRecordEdits,
      getEntityRecordNonTransientEdits = _select3.getEntityRecordNonTransientEdits,
      getEditedEntityRecord = _select3.getEditedEntityRecord; // editEntityRecord
  // deleteEntityRecord
  //


  var _dispatch6 = dispatch('core'),
      editEntityRecord = _dispatch6.editEntityRecord,
      receiveEntityRecords = _dispatch6.receiveEntityRecords;
  ...
  function storeTest() {
    ...
    var nonTransientEdits = getEntityRecordNonTransientEdits('postType', postType, postId); // yield external_wp_data_["controls"].select('core', 'getEntityRecordNonTransientEdits', kind, name, recordId);
  ...
  function beforeLanguageSwitch(lang) {
    ...
    forEach$1(supportedKeys, function (attr) {
      var rawValue = getRaw(attr);
      var contentValue = getLangContent(rawValue, lang);
  ...
  function afterLanguageSwitch() {
    ...
    var postType = getCurrentPostType();
    var postId = getCurrentPostId();
    var nonTransientEdits = getEntityRecordNonTransientEdits('postType', postType, postId);
    var edits = reduce(keys(nonTransientEdits), function (acc, attr) {
      return set(acc, attr, updateStorage[attr]);
    }, {});
  ...
  subscribe(function () {
    if (isSavingPost()) {
      ...
      if (enableDebug$2) Zubug.info('{isSavingPost}');
    } else {
      if (!postSaved) {
        if (enableDebug$2) Zubug.info('~Post Saved~');
  ...
  function editDetails(message) {
    if (enableDebug$2 && message) Zubug.info("called from {\"".concat(message, "\"}"));
    var postType = getCurrentPostType();
    var postId = getCurrentPostId();
    ...
    forEach(supportedAtts, function (selector, attr) {
      if (onlyAtts === null || includes$1(onlyAtts, attr)) {
        ...
        if (rawValue !== undefined) {
          var currentValue = getInputValue(selector);
