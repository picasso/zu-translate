var zutranslate_blocks = (function (exports) {
  ...
  var _lodash$5 = lodash,
      ...
      pick = _lodash$5.pick,
      cloneDeep = _lodash$5.cloneDeep; // isArray, includes, pull, some, 
  ...
  var debug$2 = getDebug(enableDebug$3);
  var rootClientId = 'rawRoot'; // Attributes of the object Entity --------------------------------------------]
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      debug$2.info('-Update {Entity Attributes}', edits); // collectEdits(edits);

      var postType = getCurrentPostType();
      var postId = getCurrentPostId();
      editEntityRecord('postType', postType, postId, edits);
      debug$2.infoWithId(rootClientId, '-*{Component unWatched}');
      removeWatched$1(rootClientId);
    }
  }
  ...
  function removeWatched$1(id) {
    dispatch$1(ZUTRANSLATE_STORE).removeWatched(id);
  ...
  function afterLanguageSwitch() {
    // clientId, activateSync
    if (!shouldResetEdits) return;
    isTracking = true; // const edits = [];
  ...
  function completedTracking() {
    var watched = getWatched(); // Zubug.info(`-!watched=${watched.length}`);
  ...
  function resetEdits() {
    ...
    debugPostStatus('{resetEdits}', keys$1(nonTransientEdits));
  ...
  function addWatched(id) {
    ...
    addWatched(id);
    debug$1.infoWithId(id, "-!{Component Watched}".concat(isOriginator ? '- [originator]' : ''));
  }
  function removeWatched(id) {
    ...
    removeWatched(id);
    debug$1.infoWithId(id, '-*{Component unWatched}');
