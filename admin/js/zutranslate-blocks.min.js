var zutranslate_blocks = (function (exports) {
  ...
  var _lodash$8 = lodash,
      keys$3 = _lodash$8.keys,
      isEqual = _lodash$8.isEqual,
      get$1 = _lodash$8.get,
      omit = _lodash$8.omit,
      without = _lodash$8.without;
  ...
  function storeReducer() {
    ...
    var type = action.type,
        key = action.key,
        value = action.value;
    var prevValue = get$1(state, key, '');
    var lang = get$1(state, 'lang', ''); // use an interim value to avoid unnecessary rendering
  ...
  var store = registerStore(ZUTRANSLATE_STORE, {
    ...
    selectors: {
      ...
      getLang: function getLang(state) {
        return get$1(state, 'lang');
      },
      getHooks: function getHooks(state) {
        return get$1(state, 'hooks');
      },
      getWatched: function getWatched(state) {
        return get$1(state, 'watched');
      }
    },
    controls: {}
  });
  ...
  var _lodash$7 = lodash,
      keys$2 = _lodash$7.keys,
      forEach$4 = _lodash$7.forEach,
      includes$2 = _lodash$7.includes,
      some = _lodash$7.some,
      has$3 = _lodash$7.has,
      set$1 = _lodash$7.set;
  ...
  var _select$1 = select$2('core/editor'),
  ...
  var _lodash$6 = lodash,
      keys$1 = _lodash$6.keys,
      includes$1 = _lodash$6.includes,
      forEach$3 = _lodash$6.forEach;
  ...
  var debug$4 = getDebug(enableDebug$4);
  ...
  function getWatched() {
    return select$1(ZUTRANSLATE_STORE).getWatched();
  }
  ...
  function addWatched(id) {
    ...
    debug$4.infoWithId(id, "-+{Component Watched}".concat(isOriginator ? ' [originator]' : ''));
    addWatched(id);
  }
  ...
  function removeWatched(id) {
    ...
    if (!includes$1(watched, id)) debug$4.infoWithId(id, '-!{Repeated ID removal!}');
    debug$4.infoWithId(id, '-*{Component unWatched}');
    removeWatched(id);
  }
  ...
  function syncBlocks() {
    ...
    if (activateSync$1) {
      var hooks = getHooks();
      forEach$3(hooks, function (hook, id) {
        if (id !== clientId) {
          // always add watched ID before calling the hook, because inside the hook may be logic
          // to remove this ID from the 'watched' list
          // for example, in the language switch logic for 'non-block' attributes
  ...
  function debugSync(when, isSyncEnabled) {
    ...
    var option = isSyncEnabled ? 'enabled' : 'disabled';
    ...
    var reset = cleanUnsaved && isPostPublished ? 'reset is ' + (shouldResetEdits ? '{enabled}' : '{disabled}') : resetDisabled;
    var after = isBefore ? ", Hooks count [".concat(keys$1(getHooks()).length, "]") : isPostDirty ? ", ".concat(reset) : '';
  ...
  var _lodash$5 = lodash,
      ...
      get = _lodash$5.get,
      set = _lodash$5.set,
      forEach$2 = _lodash$5.forEach,
      every = _lodash$5.every,
      has$2 = _lodash$5.has,
      reduce = _lodash$5.reduce;
  ...
  function initEditedAttribute(attr, listener) {
    ...
    setEditedAttributes(_defineProperty({}, attr, get(atts, attr)));
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      ...
      var postType = getCurrentPostType();
      var postId = getCurrentPostId(); // definitely need to be called before 'editEntityRecord'
  ...
  var _lodash$4 = lodash,
      ...
      isString = _lodash$4.isString; // Internal dependencies
  ...
  var _lodash$3 = lodash,
      ...
      castArray = _lodash$3.castArray; // , noop, includes
  ...
  function setRawAttributes() {
    var langSetter = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
  ...
  var LangControlSetting = function LangControlSetting(_ref) {
    ...
    useEffect$1(function () {
      ...
      setRawAttributes(setLanguage); // return () => {
  ...
  var BlockEditLang = function BlockEditLang(props) {
    ...
    var replaceContent = useCallback(function (lang) {
      ...
      if (lang !== prevLang) {
        ...
        debug.info("-#{switch} RAW [".concat(translatedAtts, "] for lang {").concat(lang, "}"), atts);
