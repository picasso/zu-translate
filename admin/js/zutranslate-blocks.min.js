var zutranslate_blocks = (function (exports) {
  ...
  var supportedData = getExternalData('supported', {}); // export const editorLang = getExternalData('lang', 'en');
  // const delimiters = ['[]', '{}', '<!-- -->'];
  ...
  var enableDebug$4 = getExternalData('debug.raw_store', false); // Create and register Zu Translate store -------------------------------------]
  ...
  var enableDebug$3 = getExternalData('debug.edited_entity', false); // hasChangedContent,
  ...
  function updateEntityAttributes(edits) {
    if (!isEmpty(edits)) {
      if (enableDebug$3) Zubug.info('Update Entity Attributes', edits);
      collectEdits(edits);
      var postType = getCurrentPostType();
      var postId = getCurrentPostId();
  ...
  function afterLanguageSwitch(clientId, activateSync) {
    ...
    debugPostStatus('afterLanguageSwitch', {
      clientId: clientId,
  ...
  subscribe(function () {
    if (isArray(shouldResetEdits) && isPostDirty) {
      ...
      debugPostStatus('{shouldResetEdits}', {
        shouldResetEdits: shouldResetEdits,
        nonTransientEdits: nonTransientEdits
      });

      if (!isEmpty(nonTransientEdits)) {
        pull.apply(void 0, [shouldResetEdits].concat(_toConsumableArray(keys$1(nonTransientEdits))));
  ...
  function emulateSavingPost() {
    if (enableDebug$3) Zubug.info('*Emulate {Saving Post}');
    var postType = getCurrentPostType();
    var postId = getCurrentPostId();
    var updatedRecord = getEditedEntityRecord('postType', postType, postId);
    var nonTransientEdits = getNonTransientEdits(postType, postId); //getEntityRecordNonTransientEdits('postType', postType, postId);
    ...
    if (enableDebug$3) Zubug.data({
      nonTransientEdits: nonTransientEdits,
      record: pick(updatedRecord, keys$1(nonTransientEdits))
    });
    receiveEntityRecords('postType', postType, updatedRecord, undefined, true, edits);
  }
  function storeTest() {
    Zubug.info('?storeTest - Emulate saving post');
    emulateSavingPost(); // const postType = getCurrentPostType();
  ...
  subscribe(function () {
    if (isSavingPost$1()) {
      postSaved = false;
      if (enableDebug$3) Zubug.info('?{Saving Post...}');
    } else {
      if (!postSaved) {
        if (enableDebug$3) Zubug.info('*{Post Saved}');
        postSaved = true;
      }
    }
  });
  ...
  function debugPostStatus(message, more) {
    if (enableDebug$3) {
      ...
      if (more) Zubug.info(info, cloneDeep(more));else Zubug.info(info);
    }
  ...
  var activateSync$1 = getExternalData('sync', false); // Custom hooks & helpers for 'store' -----------------------------------------]
  ...
  function useLangHook(clientId, updater) {
    useEffect$2(function () {
      var _dispatch5 = dispatch(ZUTRANSLATE_STORE),
          setHook = _dispatch5.setHook,
          removeHook = _dispatch5.removeHook;
  ...
  function syncBlocks(clientId) {
    if (activateSync$1) {
      ...
      if (enableDebug$2) {
        Zubug.info("Sync initiated from {".concat(clientId, "}"), {
      ...
      forEach$1(hooks, function (hook, id) {
        if (id !== clientId) {
          if (enableDebug$2) Zubug.info("calling hook for {".concat(id, "}"));
          hook();
        }
      });
    }
  ...
  var enableDebug$1 = getExternalData('debug.raw_helpers', false); // helpers for RAW attributes -------------------------------------------------]
    ...
    forEach(supportedAtts, function (selector, attr) {
      if (addListeners) {
        ...
        if (value === undefined) {
          ...
          if (enableDebug$1) Zubug.info("set for {".concat(attr, "} is ignored, current value = ").concat(value));
        }
      } else {
  ...
  function attachInsertedHooks(attr, selector) {
    whenNodeInserted(sidebarRoot, selector, function () {
      if (enableDebug$1) Zubug.info("Node Inserted for {\"".concat(attr, "\"}")); // synchronize switching for newly created element
  ...
  var LangControlSetting = function LangControlSetting(_ref) {
    ...
    var editorLang = useOnLangChange(switchRawAttributes);
  ...
  var BlockEditLang = function BlockEditLang(props) {
    ...
    var editorLang = useOnLangChange(replaceContent); // register 'forceUpdate' for subsequent language synchronization
