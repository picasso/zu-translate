var zutranslate_blocks = (function (exports) {
  ...
  var _wp$data$1 = wp.data,
      ...
      dispatch = _wp$data$1.dispatch; // subscribe,

  var _wp = wp,
      apiFetch = _wp.apiFetch; // Internal dependencies
  var enableDebug$2 = getExternalData('debug.post_saving', false); // Custom hooks & helpers for 'store' -----------------------------------------]
  ...
  var _lodash$3 = lodash,
      ...
      has$1 = _lodash$3.has,
      noop = _lodash$3.noop;
  var select$1 = wp.data.select; // Internal dependencies
  var enableDebug$1 = getExternalData('debug.raw_helpers', false);
  var activateSync$1 = getExternalData('sync', false); // helpers for RAW attributes -------------------------------------------------]
  ...
  function syncBlocks(clientId) {
    if (activateSync$1) {
      ...
      forEach(hooks, function (hook, id) {
        if (id !== clientId) {
          if (enableDebug$1) Zubug.info("calling hook for {".concat(id, "}"));
          hook();
        }
      });
    }
