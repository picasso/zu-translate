(function () {
  ...
  */
  ...
  function messageWithError(message, params) {
    if (_$1.isNil(params)) return message;
    params = _$1.isArray(params) || _$1.isPlainObject(params) ? toJSON(params) : String(params);
    params = params.replace(/([{|}])/g, ' $1 ').replace(/,\s*/g, ',  ').replace(/"([^"]+)":/g, '<b>$1</b>: ');
    return message.replace(/[:|.]\s*$/g, '') + ": <span class=\"zukit-data\">".concat(params, "</span>");
  ...
  function availablePanels(panels, options) {
    return pickBy$1(panels, function (p) {
